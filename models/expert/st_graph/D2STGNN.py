...
class D2STGNN(nn.Module):
    ...
    def __init__(
        ...
        self._in_feat = num_feat
        self._hidden_dim = num_hidden
        self._node_dim = node_hidden
        self._time_emb_dim = time_emb_dim
        self._k_s = k_s
        self._k_t = k_t
        self._num_layers = layer
        self.gap = gap
        ...
        self.T_i_D_emb = nn.Parameter(torch.empty(96, self._time_emb_dim))
        self.D_i_W_emb = nn.Parameter(torch.empty(7, self._time_emb_dim))

        self.layers = nn.ModuleList([DecoupleLayer(self._hidden_dim, node_hidden, time_emb_dim, adjs, k_s, k_t, dropout, input_dim, gap, self.use_pre, self.dy_graph, self.sta_graph, fk_dim=self._forecast_dim)])
        for _ in range(self._num_layers - 1):
            self.layers.append(DecoupleLayer(self._hidden_dim, node_hidden, time_emb_dim, adjs, k_s, k_t, dropout, input_dim, gap, self.use_pre, self.dy_graph, self.sta_graph, fk_dim=self._forecast_dim))

        if self.dy_graph:
            self.dynamic_graph_constructor = DynamicGraphConstructor(self._k_s, self._k_t, self._hidden_dim, self._node_dim, input_dim, dropout, time_emb_dim, adjs)
    ...
    def _prepare_inputs(self, history_data):
        num_feat = self._in_feat

        node_emb_u = self.node_emb_u
        node_emb_d = self.node_emb_d

        time_in_day_feat = self.T_i_D_emb[(history_data[:, :, :, num_feat] * 96).type(torch.LongTensor)]
    ...
    def forward(self, history_data):
        ...
        history_data = history_data.transpose(1, 3) # (b, f, n, t) -> (b, t, n, f)
        ...
        return forecast.transpose(1, 2).unsqueeze(-1).permute(0, 3, 2, 1)


class DecoupleLayer(nn.Module):
    def __init__(self, hidden_dim, node_hidden, time_emb_dim, adjs, k_s, k_t, dropout, seq_len, gap, use_pre, dy_graph, sta_graph, fk_dim=256):
        super().__init__()
        self.estimation_gate = EstimationGate(node_emb_dim=node_hidden, time_emb_dim=time_emb_dim, hidden_dim=64)
        self.dif_layer = DifBlock(hidden_dim, adjs, k_s, k_t, dropout, seq_len, gap, use_pre, dy_graph, sta_graph, forecast_hidden_dim=fk_dim)
        self.inh_layer = InhBlock(hidden_dim, dropout, seq_len, gap, forecast_hidden_dim=fk_dim)
...
class DifBlock(nn.Module):
    def __init__(self, hidden_dim, adjs, k_s, k_t, dropout, seq_len, gap, use_pre=None, dy_graph=None, sta_graph=None, forecast_hidden_dim=256):
        super().__init__()
        self.pre_defined_graph = adjs

        self.localized_st_conv = STLocalizedConv(hidden_dim, k_s, k_t, dropout, pre_defined_graph=self.pre_defined_graph, use_pre=use_pre, dy_graph=dy_graph, sta_graph=sta_graph)

        self.forecast_branch = DifForecast(hidden_dim, k_t, seq_len, gap, forecast_hidden_dim=forecast_hidden_dim)
...
class STLocalizedConv(nn.Module):
    def __init__(self, hidden_dim, k_s, k_t, dropout, pre_defined_graph=None, use_pre=None, dy_graph=None, sta_graph=None):
        super().__init__()
        self.k_s = k_s
        self.k_t = k_t
...
class DifForecast(nn.Module):
    def __init__(self, hidden_dim, k_t, seq_len, gap, forecast_hidden_dim=None):
        super().__init__()
        self.k_t = k_t
...
class InhBlock(nn.Module):
    def __init__(self, hidden_dim, dropout, seq_len, gap, num_heads=4, bias=True, forecast_hidden_dim=256):
        super().__init__()
        self.num_feat = hidden_dim
        self.hidden_dim = hidden_dim

        self.pos_encoder = PositionalEncoding(hidden_dim, dropout)
        self.rnn_layer = RNNLayer(hidden_dim, dropout)
        self.transformer_layer = TransformerLayer(hidden_dim, num_heads, dropout, bias)

        self.forecast_block = InhForecast(hidden_dim, forecast_hidden_dim, seq_len, gap)
...
class InhForecast(nn.Module):
    def __init__(self, hidden_dim, fk_dim, seq_len, gap):
        super().__init__()
        self.output_seq_len = seq_len
        self.gap = gap
...
class DynamicGraphConstructor(nn.Module):
    def __init__(self, k_s, k_t, num_hidden, node_hidden, input_dim, dropout, time_emb_dim, adjs):
        super().__init__()
        self.k_s = k_s
        self.k_t = k_t
        ...
        self.distance_function = DistanceFunction(num_hidden, node_hidden, input_dim, dropout, time_emb_dim)
        self.mask = Mask(adjs)
...
class DistanceFunction(nn.Module):
    def __init__(self, num_hidden, node_hidden, input_dim, dropout, time_emb_dim):
        super().__init__()
        self.hidden_dim = num_hidden
        self.node_dim = node_hidden
        self.time_slot_emb_dim = self.hidden_dim
        self.input_seq_len = input_dim
        self.dropout = nn.Dropout(dropout)
