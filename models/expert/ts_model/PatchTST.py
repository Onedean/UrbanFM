...
class TSTiEncoder(nn.Module):  #i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        # Encoder
        self.encoder = TSTEncoder(q_len, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm, attn_dropout=attn_dropout, dropout=dropout, pre_norm=pre_norm, activation=act, res_attention=res_attention, n_layers=n_layers, store_attn=store_attn)
...
class PatchTST(nn.Module):
    ...
    def __init__(
        ...
        # load parameters
        c_in = node_num
        context_window = input_dim
        target_window = output_dim
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.decomposition:
            ...
            x = self.model(x)
            x = x.permute(0,2,1)    # x: [Batch, Input length, Channel]
        return x.unsqueeze(-1).permute(0, 3, 2, 1)
